...
- The German rebrand has a pricing page at `/pricing/` (or `/de/pricing/` in the combined site), linked as `Preise` in the header. Its `PricingPage` component lives in `src/GermanSite.jsx`; it includes the communication → input → 36-hour Stage 1 delivery → 90-second result video and documentation flow. Stage 1 starts at €990; Stage 2 has a conditional discount (currently phrased as confirmation of Stage 1 assumptions); the bundle discounts both stages and includes an SLA for future input changes. Discount amounts, Stage 2 prices, and specific SLA terms are not yet supplied.
