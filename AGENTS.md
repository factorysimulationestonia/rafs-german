...
- Brand source material lives in the tracked `new-german-brand/` folder on the shared `german-rebrand-v2` branch after explicit approval from Hans. Preserve those supplied files as references and do not include them in a production release without explicit approval.
...
- Optimized rebrand page imagery lives under `public/de-v2/`; preserve the original concept images in `new-german-brand/` as source references.
- The current German hero (`public/de-v2/steven-hero-v2.png`) and process photo (`public/de-v2/steven-laptop-v2.png`) are retained as PNGs without optimization per user request; Hero v2 was restored on explicit user request because the v3 edit altered the face; preserve the original face and do not use v3. Keep the hero photo's embedded text visible, with the image above the page copy on mobile; the mobile 2.1:1 crop keeps the top of the image and its embedded text visible. The laptop, Estonia bog and KUKA services photos use matching responsive widths capped at 520px and a portrait 4:5 ratio. The KUKA and bog photos share the same desktop left inset and image-column width; the services text follows the image with the section grid gap. The Estonia section uses the supplied `public/de-v2/estonia-bog.jpeg` without a text overlay. Team portraits and Peter share a responsive 189.75–316.25px width (increased a further 10%, capped by the available column width) and 4:5 aspect ratio.
...
- The German rebrand has a pricing page at `/pricing/` (or `/de/pricing/` in the combined site), linked as `Preise` in the header. Its `PricingPage` component lives in `src/GermanSite.jsx`; it includes the communication → input → 48-hour Phase 1 delivery after receipt of complete input data → 90-second result video and documentation flow. Phase 1 has a fixed price of €990 (not a starting price); Phase 1 provides a documented initial assessment, input data and model assumptions, initial simulation results with limitations and recommended next steps, plus a 90-second video. Phase 2 has a discount after completing Phase 1 (without the qualifier möglich; no promise to credit the entire Phase 1 fee); the bundle discounts both stages and includes an SLA for future input changes. Use German pricing terminology: Phase 1/2, Gesamtpaket, Angaben, Ingenieurarbeit, and Betreuungsvereinbarung instead of Stage, Bundle, Input, Engineering, and SLA. Discount amounts, Phase 2 prices, and specific support terms are not yet supplied.

## Project

- The German landing page has a services section immediately after “So arbeiten wir”, summarizing automation feasibility and factory concept validation. Both provide initial simulation findings within 48 hours after complete input data, with assumptions, limitations, risks, next steps and a 90-second video. Its supplied KUKA robot photo is `public/de-v2/services-kuka-v2.webp` (background pallet-jack handle removed with imagegen; original v1 preserved), displayed to the left of the text on desktop; it replaces the generated illustration and has no AI-generated caption.

- German direct-load regression coverage: `npm run build:de` generates directory `index.html` files for all known routes and `404.html` via `scripts/german-pages.mjs`; update its route list when adding pages. Vite uses explicit `appType: 'spa'`. Run `npm run test:routing` after building to verify direct navigation and cache-disabled refresh with Chrome against German Vite mode and a Pages-style static artifact server. Keep the `/rafs-german/` base and clean paths; do not replace them with hash routing.
- The current GitHub Pages target uses Vite base `/rafs-german/` and React Router basename `/rafs-german` derived from `BASE_URL`. German internal links use React Router `Link` with paths excluding the base; `useLocation` drives route rendering and hash scrolling. `.github/workflows/deploy.yml` copies `dist/index.html` to `dist/404.html` after the German build and before upload. Preserve clean URLs and verify with `node scripts/check-german-routing.mjs`.
...
- German FAQ answers contain multiple paragraphs inside a nested content column. Keep the legal-page negative paragraph margins scoped to direct section children (`section > p + p`) so FAQ paragraphs do not overlap; preserve positive spacing between FAQ paragraphs on mobile as well.

- The German hero uses wrapping, smaller headings on 1121–1919px screens to avoid covering the portrait. At 1120px and below, photo and copy stack; mobile spacing is compact. Preserve the wide desktop composition above this range.
...
- The German landing-page contact area and every German footer show the DACH contact from the centralized `dachContact` object in `src/main.jsx`. Peter J. Fischer is the confirmed DACH contact, including his LinkedIn profile; his supplied portrait with a background matched to Hans is stored at `public/team/peter-fischer-de-grey.jpg` and displayed in the German landing-page contact area.
- The German team includes Triinu Strandberg as `Vertrags- und Dokumentenmanagerin`. German portraits for Steven, Markus, Triinu, and Peter use the `public/team/*-de-grey.jpg` files with soft warm grey backgrounds matched to Hans. Original portraits are preserved; ET/EN retain their existing team data and portraits. Triinu is not currently listed on the ET/EN site.
