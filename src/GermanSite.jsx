...
function BrandLogo({ inverted = false }) {
  return (
    <span className={`de2-logo${inverted ? ' de2-logo--inverted' : ''}`} aria-label="Factory Simulation">
      <span className="de2-logo__mark" aria-hidden="true"><i /><i /><i /></span>
      <span className="de2-logo__name">Factory<br />Simulation</span>
    </span>
  );
}
...
function BrandHeader({ homePath, sectionHref }) {
  ...
  return (
    <header className="de2-header">
      <a className="de2-header__brand" href={homePath} aria-label="Factory Simulation Startseite"><BrandLogo /></a>
  ...
  return (
    <footer className="de2-footer">
      <div className="de2-footer__main">
        <a href={homePath} aria-label="Factory Simulation Startseite"><BrandLogo inverted /></a>
...
export default function GermanSite({ assetPath, basePath, contactEndpoint, contactPerson, isDedicatedSite = false, LinkedinIcon, onLeadConversion, privacyDetails, t }) {
  ...
  }, [faqPath, germanOrigin, homePath, impressumPath, isFaq, isImpressum, isLegalNotice, isLegalPage, isPrivacy, legalNoticePath, privacyPath, resolvedMainSiteOrigin, t.faq.metaDescription, t.faq.metaTitle]);
    ...
    <div className="de2-site">
      ...
      <main>
        ...
        <section className="de2-process" id="prozess">
          <div className="de2-process__intro"><p className="de2-eyebrow">So arbeiten wir</p><h2>Von Ihrer Herausforderung zur validierten Lösung. Schnell.</h2><p>Keine unnötigen Schleifen, keine unklare Übergabe. Sie sprechen direkt mit dem Team, das Ihr Modell baut und die Ergebnisse bewertet.</p><div className="de2-process__steps">{processSteps.map(([number, title, text]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div><a className="de2-text-link" href="#kontakt">Unser Prozess <ArrowIcon /></a></div>
          <div className="de2-process__visual"><img src={assetPath('/de-v2/how-we-work.webp')} alt="Produktionssimulation im Engineering-Prozess" loading="lazy" decoding="async" /></div>
        </section>

        <section className="de2-team" id="team">
          <div className="de2-estonia">
            <img src={assetPath('/de-v2/tallinn.webp')} alt="Tallinn, Estland" loading="lazy" decoding="async" />
            ...
            <blockquote>„Ich habe Factory Simulation gegründet, um Engineering schlanker, flexibler und näher an den Menschen zu machen, mit denen wir arbeiten.“<strong>Steven Strandberg</strong><span>Gründer & Simulationsingenieur</span></blockquote>
          </div>
        ...
        <section className="de2-contact" id="kontakt">
          <div className="de2-contact__intro"><p className="de2-eyebrow">Projektstart</p><h2>Was möchten Sie sicher entscheiden?</h2><p>Beschreiben Sie kurz Ihre Produktionsfrage. Wir melden uns persönlich und klären, welche Daten und welcher Simulationsumfang wirklich sinnvoll sind.</p>{contactPerson && <address><span>Ihr Ansprechpartner für DACH</span><h3>{contactPerson.name}</h3><p>{contactPerson.role}</p><a href={contactPerson.phoneHref}><PhoneIcon />{contactPerson.phone}</a><a href={`mailto:${contactPerson.email}`}><MailIcon />{contactPerson.email}</a>{contactPerson.linkedin && LinkedinIcon && <a className="de2-contact__social" href={contactPerson.linkedin} target="_blank" rel="noreferrer" aria-label={`${contactPerson.name} LinkedIn`}><LinkedinIcon /></a>}</address>}</div>
